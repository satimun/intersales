...
{
    public class ShipmentPlanVsActualReport : BaseAPIEngine<ShipmentPlanGetReport2Req, ShipmentPlanGetReport2Res>

    {
        ...
        private List<ShipmentPlanGetReport2Res.Shipment> ConvertData(List<ShipmentPlanGetReport2> data)
        {
            ...
            data.GroupBy(x => new { id = x.HID, zone = x.ZoneAccount_ID }).ToList().ForEach(m =>
            {
                var x = m.ToList();
