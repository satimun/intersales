...
{
    public class PriceStdSearchPriceRangeHAPI : BaseAPIEngine<SearchRequest, PriceStdSearchPriceRangeHResponse>
    {
        protected override string PermissionKey { get { return "PUBLIC_API"; } }

        protected override void ExecuteChild(SearchRequest dataReq, PriceStdSearchPriceRangeHResponse dataRes)
        {
            PriceStdADO.GetInstant().SearchPriceRangeH(dataReq, this.Logger).ForEach(
                   x =>
                   {
                       dataRes.priceStdRangeHs.Add(new PriceStdRangeHs()
                       {
                           id = x.ID
                           , priceStdMainID = x.PriceStdMain_ID
                           , minTwineSize = new PriceStdRangeHs.TwineSize() { code = x.MinProductTwineSizeCode, size = x.MinFilamentSize, amount = x.MinFilamentAmount, word = x.MinFilamentWord??"" }
                           , maxTwineSize = new PriceStdRangeHs.TwineSize() { code = x.MaxProductTwineSizeCode, size = x.MaxFilamentSize, amount = x.MaxFilamentAmount, word = x.MaxFilamentWord??"" }
