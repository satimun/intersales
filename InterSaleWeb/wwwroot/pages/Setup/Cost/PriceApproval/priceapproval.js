'use strict';
app.controller("PriceApprovalCtrl", function ($rootScope, $scope, common, $filter, $timeout, API) {
    ...
    $scope.countryGroup = {
        list: []
        , selected: []
        , filter: (query) => { return $filter('filter')($scope.countryGroup.list, { 'text': query }); }
    };
    ...
    API.CountryGroup.SearchCountry({
        ...
        callback: (res) => {
            res.data.countryGroups.forEach((v) => {
                $scope.countryGroup.list.push({ id: v.id, code: v.code, text: common.GetCodeDescription(v) });
            });
        },
    ...
    $scope.IP_DB = $rootScope.IP_DB + 'sxsPriceStdMain, sxsPriceStdEffectiveDate';
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'countryGroup_view', display: 'Country Group', width: { min: 100 }, multiLine: true }));

    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'priceStdEffectiveDate.code', display: 'Code', width: { default: 70 }, group: { name: 'effective', display: 'Effective', langCode: '' } }));
    ...
    $scope.gridOpt.onRegisterApi = function (gridApi) {
        ...
        gridApi.cellNav.on.navigate($scope, (newRowCol, oldRowCol) => {
            if (!oldRowCol || oldRowCol.row.uid !== newRowCol.row.uid) {
                ...
                $rootScope.PriceApprovalGridDetailCtrl_SetGrid(flag);
                $rootScope.PriceApprovalGridDetailCtrl_SetData(newRowCol.row.entity, $scope.statusFlag.id);
            }
        });
    ...
    $scope.LoadData = () => {
        API.PriceStd.SearchHeader({
            data: { countryGroupIDs: $scope.countryGroup.selected.map(x => x.id), dateFrom: common.GetDateString($scope.dtpFrom), dateTo: common.GetDateString($scope.dtpTo), status: $scope.statusFlag.id },
            callback: (res) => {
                res.data.priceHeaders.forEach((d) => {
                ...
                $scope.gridOpt.data = res.data.priceHeaders;
                $scope.gridApi.grid.refresh();
                $rootScope.PriceApprovalGridDetailCtrl_SetData(0, 0, 0);
            },
            error: (res) => { common.AlertMessage("Error", res.message); }
        });
    };
...
app.controller("PriceApprovalGridDetailCtrl", function ($rootScope, $scope, common, dialog, API) {
    ...
    $scope.IP_DB = $rootScope.IP_DB + 'sxsPriceStdRangH, sxsPriceStdRangD, sxsPriceStdProd, sxsPriceStdValue';
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'fob', display: 'FOB', width: { min: 60 }, format: { type: 'decimal', scale: 2, summary: 'none' }, setclass: GridClass, group: { name: 'price', display: 'Price', langCode: '' } }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'caf', display: 'C&F', width: { min: 60 }, format: { type: 'decimal', scale: 2, summary: 'none' }, setclass: GridClass, group: { name: 'price', display: 'Price', langCode: '' } }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'cif', display: 'CIF', width: { min: 60 }, format: { type: 'decimal', scale: 2, summary: 'none' }, setclass: GridClass, group: { name: 'price', display: 'Price', langCode: '' } }));
    ...
    $scope.gridOpt.onRegisterApi = function (gridApi) {
        $scope.gridApi = gridApi;
        gridApi.selection.on.rowSelectionChanged($scope, (row) => { console.log(row); $scope.ChkChange(); });
        gridApi.cellNav.on.navigate($scope, (newRowCol, oldRowCol) => {
            if (!oldRowCol || oldRowCol.row.uid !== newRowCol.row.uid) {
                $rootScope.PriceApprovalGridDiscountCtrl_SetData(newRowCol.row.entity, $scope.main);
            }
        });
    };       
    ...
    $rootScope.PriceApprovalGridDetailCtrl_SetData = (main, status) => {
        $scope.main = main;
        $scope.status = status;
        if (!main) {
            $scope.gridOpt.data = [];
        ...
        API.PriceStd.SearchDetail({
            data: { mainID: main.id, effectiveID: main.priceStdEffectiveDate.id, status: status },
            callback: (res) => {
                res.data.priceDetails.forEach((d) => {
                ...
                $scope.gridOpt.data = res.data.priceDetails;
                $scope.gridApi.grid.refresh();
                $rootScope.PriceApprovalGridDiscountCtrl_SetData();
            },
            error: (res) => { common.AlertMessage("Error", res.message); }
        });
    };
    ...
    $scope.Approval = (flag) => {
        ...
        API.PriceStd.Approval({
            data: { priceDetails: data },
            callback: (res) => {
                common.AlertMessage('Success', '').then((ok) => { $rootScope.PriceApprovalGridDetailCtrl_SetData($scope.main, $scope.status); });
            },
            error: (res) => {
                var msg = '';
                res.data.priceDetails.forEach((v) => {
...
app.controller("PriceApprovalGridDiscountCtrl", function ($rootScope, $scope, common, intersales, API) {

    $scope.IP_DB = $rootScope.IP_DB + 'sxsDiscountStdMain, sxsDiscountStdRangH, sxsDiscountStdRangD, sxsDiscountStdProd, sxsDiscountStdValue';
    ...
    var SetClass = function (grid, row, col) {
        ...
        if (/^AN./.test(row.entity.approvedStatus)) {
            return 'text-danger';
        }
        return classx;
    };
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'numRow', display: 'No', width: { min: 55, max: 65 }, format: { type: 'numRow' }, setclass: SetClass }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'customerDescription', display: 'Customer', width: { min: 170 }, setclass: SetClass }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'discountStdTableCode', display: 'code', width: { min: 150 }, group: { name: 'disx', display: 'Discount Detail', langCode: '' }, setclass: SetClass }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'salesDescription', display: 'Sales Description', width: { min: 250 }, group: { name: 'disx', display: 'Discount Detail', langCode: '' }, setclass: SetClass }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'tagDescription', display: 'Tag Description', width: { min: 250 }, group: { name: 'disx', display: 'Discount Detail', langCode: '' }, setclass: SetClass }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'effectiveDateFrom', display: 'From', format: { type: 'datetime' }, width: { min: 90 }, group: { name: 'priceeff', display: 'Discount Effective Date', langCode: '' }, setclass: SetClass }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'effectiveDateTo', display: 'To', format: { type: 'datetime' }, width: { min: 90 }, group: { name: 'priceeff', display: 'Discount Effective Date', langCode: '' }, setclass: SetClass }));

    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'discountPercent', display: '% Discount', width: { min: 99 }, format: { type: 'decimal', scale: 2, summary: 'none' }, setclass: SetClass, multiLine: true }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'discountAmount', display: 'Fix Discount', width: { min: 99 }, format: { type: 'decimal', scale: 2, summary: 'none' }, setclass: SetClass, multiLine: true }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'increaseAmount', display: 'Surcharge', width: { min: 99 }, format: { type: 'decimal', scale: 2, summary: 'none' }, setclass: SetClass, multiLine: true }));

    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'fob', display: 'HC', width: { min: 70 }, format: { type: 'decimal', scale: 2, summary: 'none' }, group: { name: 'fob', display: 'FOB', langCode: '' }, setclass: SetClass }));
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'caf', display: 'HC', width: { min: 70 }, format: { type: 'decimal', scale: 2, summary: 'none' }, group: { name: 'caf', display: 'C&F', langCode: '' }, setclass: SetClass }));
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'cif', display: 'HC', width: { min: 70 }, format: { type: 'decimal', scale: 2, summary: 'none' }, group: { name: 'cif', display: 'CIF', langCode: '' }, setclass: SetClass }));
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'approvedStatus', display: 'Approved Status', width: { default: 200 }, setclass: SetClass, hiding: true, multiLine: true }));
    ...
    $rootScope.PriceApprovalGridDiscountCtrl_SetData = (row, main) => {
        if (!row || !main) {
            $scope.gridOpt.data = [];
            $scope.gridApi.grid.refresh();
            return;
        ...
        API.DiscountStd.SearchDiscountForPrice({
            ...
            callback: (res) => {
                res.data.discounts.forEach((r) => {
                    r.effectiveDateFrom = common.CreateDateTime(r.effectiveDateFrom);
                    r.effectiveDateTo = common.CreateDateTime(r.effectiveDateTo);
                    r.fob = row.fob;
                    r.caf = row.caf;
                    r.cif = row.cif;
                    r.restFOB = intersales.PriceToRest(r.fob, r.discountPercent, r.discountAmount, r.increaseAmount);
                    r.restCAF = intersales.PriceToRest(r.caf, r.discountPercent, r.discountAmount, r.increaseAmount);
                    r.restCIF = intersales.PriceToRest(r.cif, r.discountPercent, r.discountAmount, r.increaseAmount);
                });

                $scope.gridOpt.data = res.data.discounts;
                $scope.gridApi.grid.refresh();
            },
            error: (res) => { common.AlertMessage("Error", res.message); }
        });
