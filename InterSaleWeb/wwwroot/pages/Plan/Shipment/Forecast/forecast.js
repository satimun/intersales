'use strict';
app.controller("shipmentForecastReportController", function ($rootScope, $scope, $location, $timeout, $filter, $q, common, intersales, API) {
    ...
    $scope.GenMonth = (year) => {
        if (year) {
            ...
            $rootScope.shipmentForecastReportGridCtrl_SetColumn(parseInt($scope.yearFrom), parseInt($scope.yearTo), $scope.ytd);
        }
    };
    ...
    $scope.ytdChk = (ytd) => {
        ...
        if (ytd) {
            $scope.yearFrom = d.getFullYear();
            $scope.yearTo = d.getFullYear();
            dateFrom = new Date($scope.yearFrom, 0, 1);
        } else {
        ...
        $scope.monthFrom.view = dateFrom.toLocaleString("en-us", { month: "long" });
        $timeout(() => {
            $scope.monthTo.view = dateTo.toLocaleString("en-us", { month: "long" });
        });
    };
    ...
    $scope.monthTo.SetID = function (id) {
        $scope.monthTo.id = id;
        try {
            $scope.dtpTo = new Date(parseInt($scope.yearTo), parseInt(id), 0);
        } catch (ex) {
            console.log(ex);
        }
    ...
    $scope.genMonth = (year) => {
        ...
        for (var i = 0; i < 12; i++) {
            ...
            $scope.monthFrom.list.push({ id: i + 1, view: date.toLocaleString("en-us", { month: "long" }) });
        }
    ...
    $scope.monthFrom.SetID = function (id) {
        ...
        try {
            $scope.dtpFrom = new Date(parseInt($scope.yearFrom), parseInt(id) - 1, 1);
        } catch (ex) {
            console.log(ex);
        }
    ...
    $scope.LoadData = function () {
        ...
        $scope.dtpFrom = new Date(parseInt($scope.yearFrom), parseInt($scope.monthFrom.id) - 1, 1);
        $scope.dtpTo = new Date(parseInt($scope.yearTo), parseInt($scope.monthTo.id), 0);
        ...
        var showCol = $rootScope.shipmentForecastReportGridCtrl_ShowHideCol();

        API.ShipmentPlan.GetForecastReport2({
            data: {
            ...
            callback: function (res) {
                res.data.shipmentPlanForecasts.forEach((row) => {
                    ...
                    //row.customer.code_view = row.customer.code;
                ...
                $rootScope.shipmentForecastReportGridCtrl_SetData();
        ...
        //KSSClient.API.ShipmentPlan.GetForecastReport({
        //    data: {
        //        dateFrom: common.GetDateString($scope.dtpFrom)
        //        , dateTo: common.GetDateString($scope.dtpTo)
        //        , costID: $scope.cost.id
    ...
    $rootScope.shipmentForecastReport_GetDetail = (data) => {
        var deferred = $q.defer();
        KSSClient.API.ShipmentPlan.GetActual({
            data: {
                dateFrom: common.GetDateString($scope.dtpFrom)
                , dateTo: common.GetDateString($scope.dtpTo)
                , zoneID: data.zoneID
    ...
    $scope.Export = function () {
        ...
        // Report Name
        //data.push([{ value: 'Forecast vs Shipment Plan vs Actual Report', metadata: { style: reportNamme, type: 'string' } }]);
        data.push([{ value: 'Forecast vs Shipment Plan vs Actual Report : ' + ' ( ' + $scope.dataH.monthFrom + ' - ' + $scope.dataH.monthTo + ' ' + ($scope.ytd ? '' : $scope.dataH.year) + ' )', metadata: { style: reportNamme, type: 'string' } }]);
        worksheet.setRowInstructions(1, { height: 45 });

        // title header
        //data.push([{ value: 'Forecast Year : ', metadata: { style: titleFormat, type: 'string' } }, { value: $scope.year, metadata: { style: valueFormat, type: 'string' } }
        ...
        //]);
        //data.push([{ value: 'Customers : ', metadata: { style: titleFormat, type: 'string' } }, { value: customer, metadata: { style: valueFormat, type: 'string' } }
        //    , { value: 'Sales Representative : ', metadata: { style: titleFormat, type: 'string' } }, { value: $scope.emp.view, metadata: { style: valueFormat, type: 'string' } }
        //]);
        //data.push([{ value: 'Cost : ', metadata: { style: titleFormat, type: 'string' } }, { value: $scope.cost.view, metadata: { style: valueFormat, type: 'string' } }
        //]);
        ...
        var showCols = $rootScope.shipmentForecastReportGridCtrl_ShowCols();
        ...
        //var tmpSum = {};
        var dataGrid = $rootScope.shipmentForecastReportGridCtrl_GetData();
        ...
        common.ExportExcel(workbook, 'Forecast vs Shipment Plan vs Actual Report');
    };

});

app.controller("shipmentForecastReportGridCtrl", function ($rootScope, $scope, $filter, $timeout, $uibModal, common) {
    ...
    $scope.gridOpt = common.CreateGrid2({ footer: true, _grouping2: { enable: true, showTotal: true } });
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'zone.code', display: 'Zone', width: { default: 250 }, format: { type: 'customText' }, _grouping2: true, setclass: SetClass, hiding: false, showCountItems: true }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'country.code', display: 'Country', width: { default: 250 }, format: { type: 'customText' }, _grouping2: true, setclass: SetClass, hiding: false, showCountItems: true }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'customer.code', display: 'Customer', width: { default: 250 }, format: { type: 'customText' }, _grouping2: true, setclass: SetClass, hiding: false, showCountItems: true }));
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'productType.code', display: 'Product', width: { default: 200 }, format: { type: 'customText' }, _grouping2: true, setclass: SetClass, hiding: false, showCountItems: true }));
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'diameter', display: 'Diameter', width: { default: 100 }, format: { type: 'customText' }, _grouping2: true, setclass: SetClass, hiding: false, showCountItems: true }));
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'color.code', display: 'Color', width: { default: 150 }, format: { type: 'customText' }, _grouping2: true, setclass: SetClass, hiding: false, showCountItems: true }));
    ...
    $rootScope.shipmentForecastReportGridCtrl_SetColumn = (yearFrom, yearTo, ytd) => {
        $scope.gridOpt.data = [];
        $scope.years = [];
        $scope.ClearColumn();
        if (yearFrom == yearTo || ytd) {
            ...
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'shipmentPlan.weight', display: 'Weight (KG)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'shipmentPlan', display: 'Shipment Plan', langCode: '' }, setclass: SetClass }));
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'shipmentPlan.amount', display: 'Amount (PC)', width: { default: 120 }, format: { type: 'decimal', scale: 0 }, group: { name: 'shipmentPlan', display: 'Shipment Plan', langCode: '' }, setclass: SetClass }));
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'shipmentPlan.value', display: 'Value(THB)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'shipmentPlan', display: 'Shipment Plan', langCode: '' }, setclass: SetClass }));
            ...
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'lastYear.weight', visible: false, display: 'Weight (KG)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'lastYear', display: 'Last Year', langCode: '' }, setclass: SetClass }));
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'lastYear.amount', visible: false, display: 'Amount (PC)', width: { default: 120 }, format: { type: 'decimal', scale: 0 }, group: { name: 'lastYear', display: 'Last Year', langCode: '' }, setclass: SetClass }));
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'lastYear.value', visible: false, display: 'Value(THB)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'lastYear', display: 'Last Year', langCode: '' }, setclass: SetClass }));
            ...
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'actualVsShipmentPlan.weight', display: 'Weight (KG)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'actualVsShipmentPlan', display: 'Actual VS Shipment Plan', langCode: '' }, setclass: SetClass }));
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'actualVsShipmentPlan.value', display: 'Value(THB)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'actualVsShipmentPlan', display: 'Actual  VS Shipment Plan', langCode: '' }, setclass: SetClass }));

            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'differentAcVsShipment.weight', display: 'Weight (KG)', width: { default: 120 }, format: { type: 'decimal', scale: 2, summary: 'percent', multi: 'actualVsShipmentPlan.weight', divi: 'shipmentPlan.weight' }, group: { name: 'differentAcVsShipment', display: '% Different', langCode: '' }, setclass: SetClass }));
            $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'differentAcVsShipment.value', display: 'Value(THB)', width: { default: 120 }, format: { type: 'decimal', scale: 2, summary: 'percent', multi: 'actualVsShipmentPlan.value', divi: 'shipmentPlan.value' }, group: { name: 'differentAcVsShipment', display: '% Different', langCode: '' }, setclass: SetClass }));


        } else {
            $scope.cols.find(x => x.id === 1).fieldss = [];
    ...
    $scope.AddColumn = function (year) {
        ...
        $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: `_shipmentPlan${year}.weight`, display: 'Weight (KG)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: `_shipmentPlan${year}`, display: `Shipment Plan ${year}`, langCode: '' }, setclass: SetClass }));
        $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: `_shipmentPlan${year}.amount`, display: 'Amount (PC)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: `_shipmentPlan${year}`, display: `Shipment Plan ${year}`, langCode: '' }, setclass: SetClass }));
        $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: `_shipmentPlan${year}.value`, display: 'Value(THB)', width: { default: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: `_shipmentPlan${year}`, display: `Shipment Plan ${year}`, langCode: '' }, setclass: SetClass }));
        $scope.cols.find(x => x.id === 1).fieldss.push(`_shipmentPlan${year}`);
    ...
    $scope.ShowDetail = (row) => {
        ...
        if ($scope.gColor) { req.colorID = row.entity.color.id; req.color = row.entity.color.code; dataModal.title.push({ text: 'Color', value: row.entity.color.code_vieworg }); }
        //if ($scope.gMonth) { req.month = row.entity.month; dataModal.title.push({ text: 'Month', value: row.entity.month_vieworg }); }

        $rootScope.shipmentForecastReport_GetDetail(req).then((data) => {
            dataModal.data = data;
    ...
    $rootScope.shipmentForecastReportGridCtrl_GetData = () => { return $scope.gridApi.grid; };

    $scope.isProcess = false;

    $rootScope.shipmentForecastReportGridCtrl_SetData = function () {
        var tmpObj = [], data = [];

        if ($rootScope.shipmentForecastData.length) $scope.isGroupChange = false;
        $scope.isProcess = true;

        $rootScope.shipmentForecastData.forEach((d) => {
            let group = [];
            ...
            if (!o) {
                ...
                if ($scope.years.length > 1) {
                    $scope.years.forEach(y => {
                        o.total[`_shipmentPlan${y}`] = { weight: 0, amount: 0, value: 0 };
                        o.total[`_forecast${y}`] = { weight: 0, value: 0 };
                        o.total[`_actual${y}`] = { weight: 0, amount: 0, value: 0 };
                    });
                } else {
                    o.total.shipmentPlan = { weight: 0, amount: 0, value: 0 };
                    ...
                    o.total.actualVsShipmentPlan = { weight: 0, value: 0 };
                    o.total.actualVsLastYear = { weight: 0, value: 0 };
                    o.total.differentAcVsFore = { weight: 0, value: 0 };
                    o.total.differentAcVsShipment = { weight: 0, value: 0 };
            ...
            if ($scope.years.length > 1) {
                $scope.years.forEach(y => {
                    ...
                    if (v) {
                        o.total[`_shipmentPlan${y}`].weight += v.shipmentPlan.weight;
                        o.total[`_shipmentPlan${y}`].amount += v.shipmentPlan.amount;
                        o.total[`_shipmentPlan${y}`].value += v.shipmentPlan.value;
                ...
                o.total.shipmentPlan.weight += d.shipmentPlan.weight;
                o.total.shipmentPlan.amount += d.shipmentPlan.amount;
                o.total.shipmentPlan.value += d.shipmentPlan.value;
                ...
                o.total.actualVsShipmentPlan.weight = o.total.actual.weight - o.total.shipmentPlan.weight;
                o.total.actualVsShipmentPlan.value = o.total.actual.value - o.total.shipmentPlan.value;
                ...
                o.total.differentAcVsShipment.weight = o.total.shipmentPlan.weight === 0 ? 0 : (o.total.actualVsShipmentPlan.weight / o.total.shipmentPlan.weight) * 100;
                o.total.differentAcVsShipment.value = o.total.shipmentPlan.value === 0 ? 0 : (o.total.actualVsShipmentPlan.value / o.total.shipmentPlan.value) * 100;
        ...
        $scope.colGroup.forEach((c) => {
            if (c === 'diameter') {
                if ($scope.lists.find(v => v.field === 'diameter').checked) {
                    codeSort.push('twine.size');
                    codeSort.push('twine.amount');
                    codeSort.push('twine.word');
                }
            } else /*if(c !== 'month')*/ { codeSort.push(c); }
        });
    ...
    $rootScope.shipmentForecastReportGridCtrl_ShowHideCol = () => {
        return $scope.lists;
    };
    ...
    $scope.cols = [
        { id: 1, label: 'Shipment Plan', fields: ['shipmentPlan', 'differentAcVsShipment', 'actualVsShipmentPlan'], fieldss: [], checked: true },
        ...
        { id: 3, label: 'Actual', fields: ['actual'], checked: true },
        //{ id: 4, label: 'Amount (pc)', fields: ['amount'], checked: false }
    ];

    $rootScope.shipmentForecastReportGridCtrl_ShowCols = () => {
        var colss = [], colsl = [];
        if ($scope.years.length > 1) {

            $scope.years.forEach(y => {
                if ($scope.cols.find(x => x.id === 1).checked) {
                    colss.push({ id: `_shipmentPlan${y}`, label1: 'Weight(KG)', label2: `Shipment Plan ${y}`, field: `_shipmentPlan${y}.weight` });
                    colss.push({ id: `_shipmentPlan${y}`, label1: 'Amount(PC)', label2: `Shipment Plan ${y}`, field: `_shipmentPlan${y}.amount` });
                    colss.push({ id: `_shipmentPlan${y}`, label1: 'Value(THB)', label2: `Shipment Plan ${y}`, field: `_shipmentPlan${y}.value` });
                }
    ...
    $scope.ModalOpen = (dataModal) => {
        var modalInstance = $uibModal.open({
            ...
            controller: 'ShipmentForeCastModalCtrl',
            size: 'mdc',
            appendTo: undefined,
            resolve: { dataModal: function () { return dataModal; } },
            backdrop: 'static',
            keyboard: false,
            templateUrl: 'ShipmentForeCastModal.html'
        });
...
app.controller('ShipmentForeCastModalCtrl', function ($scope, dataModal, common, $rootScope, $uibModalInstance, $filter) {
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'ciCode', display: 'KCI', width: { default: 120 }, showCountItems: true }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'ciDate', display: 'KCI Date', width: { default: 100 }, format: { type: 'datetime' } }));
    ...
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'other.weightKg', display: 'Weight (kg)', width: { min: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'other', display: 'Other', langCode: '' } }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'other.amountpc', display: 'Amount (pc)', width: { min: 120 }, format: { type: 'decimal', scale: 0 }, group: { name: 'other', display: 'Other', langCode: '' } }));
    $scope.gridOpt.columnDefs.push(common.AddColumn2({ name: 'other.value', display: 'Value', width: { min: 120 }, format: { type: 'decimal', scale: 2 }, group: { name: 'other', display: 'Other', langCode: '' } }));

    $scope.cumulative = function (grid, myRow, myCol) {
        if (grid.id === $scope.gridApi.grid.id) {
            if (myCol.field === "ciDate") {
                return KSSClient.Engine.Common.GetDateView(KSSClient.Engine.Common.CreateDateTime(myRow.entity.ciDate));
            }
        }
        return false;
    };
